...
export class NuresserviceService {
  ...
  adddressing(data) {
    ...
    return this.http.post<any>(this.Api + `adddressing`, payload).pipe(map((res) => {
  ...
  getdressing() {
    return this.http.post<any>(this.Api + `getDressing`, []).pipe(map((res) => {
      return res;
    }, (error) => {
      return error;
    }));
  ...
  drillpost(data: any) {
    ...
    const signature = this.signPayload(encryptedPayload);
    const payload = { encryptedPayload, signature };
    return this.http.post<any>(this.Api + `nursetrainingpost`, payload).pipe(map((res) => {
      return this.handleResponse(res);
    }, error => {
      this.errorAlert();
      return error;
  ...
  getnursetrainingdata() {
    return this.http.get<any>(this.Api + `getnursetrainingdata`).pipe(map(res => {
      return this.handleResponse(res);
    }, error => {
      this.errorAlert();
      return error;
