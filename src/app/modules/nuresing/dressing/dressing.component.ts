...
export class DressingComponent {
  ...
  constructor(private modalService: NgbModal, private router: Router,
    public formBuilder: FormBuilder, public datePipe: DatePipe, private myservice: NuresserviceService) {
  ...
  gotDataAllpatients() {
    this.myservice.getdressing().subscribe((res) => {
  ...
  submitForm() {
    ...
    if (this.typeValidationForm.invalid) {
      ...
      this.myservice.adddressing(data).subscribe((res: any) => {
        if (res.status == 200) {
          Swal.fire({
            position: 'top-end',
            icon: 'success',
