...
@Component({
  selector: 'app-consulationfee',
  templateUrl: './consulationfee.component.html',
  styleUrls: ['./consulationfee.component.scss']
})
export class ConsulationfeeComponent implements OnInit {
  ...
  addemployeedetailsform: FormGroup;
   ...
   nextdisplayedColumns: string[] = ['i','card_name', 'roomtype','amount'];
  ...
  headerclass = {
   ...
   submitroomtype() {
     ...
     if (this.addemployeedetailsform.invalid) {
       ...
       this.service.consultant_type(data).subscribe((res: any) => {
         if (res.status == 200) {
           Swal.fire({
             position: 'top-end',
             icon: 'success',
   ...
   casefiledata: any;
   getroomtypedata() {
     this.service.getconsultant_typetable().subscribe(res => {
       this.showSpinner = false;
   ...
   editaddroomtype() {
     if (this.updateform.invalid) {
       ...
       this.service.updateconsultant_type(data).subscribe(res => {
 ...
 changepn(event: any) {
  ...
  this.selectedRoomType = parts[0]; // "Dermatologist"
  this.selectedCardId = parts[1];   // "1"
