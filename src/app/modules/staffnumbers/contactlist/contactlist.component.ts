...
export class ContactlistComponent implements OnInit {
  ...
  nextdisplayedColumns: string[] = ['i', 'name', 'number', 'department','address','alternatenumber'];
  ...
  exportTable() {
    TableUtil.exportTableToExcel('exportTable', 'Contacts-List'); // table id, file name
  }
  ...
  submitroomtype() {
    ...
    // if (this.addemployeedetailsform.value.number !==  this.addemployeedetailsform.value.alternatenumber) {
    //   alert("Mobile Number & Alternate Mobile Number Should be different");
    // } 
     const number = this.addemployeedetailsform.value.number;
  const alternatenumber = this.addemployeedetailsform.value.alternatenumber;

  // 2️⃣ Stop if numbers are same
  if (number == alternatenumber) {
    ...
    return;
      ...
      var data = {
        ...
        number: this.addemployeedetailsform.value.number,
        department: this.addemployeedetailsform.value.department,
        address: this.addemployeedetailsform.value.address,
        alternatenumber:this.addemployeedetailsform.value.alternatenumber,
        user_id: localStorage.getItem('user_id'),
        usr_nm: localStorage.getItem('usr_nm'),
      }
      this.service.emergenctcontactlistpost(data).subscribe((res: any) => {
        if (res.status == 200) {
          Swal.fire({
            position: 'top-end',
            icon: 'success',
  ...
  casefiledata: any;
  getroomtypedata() {
    this.service.emergenctcontactlistget().subscribe(res => {
      this.showSpinner = false;
  ...
  editaddroomtype() {
    if (this.updateform.invalid) {
      ...
      this.service.editaddroomtype(data).subscribe(res => {
