...
export class AddequipmentComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal, private router: Router,
    private formBuilder: FormBuilder, private service: AssetserviceService, public dialog: MatDialog) { }
  ...
  submitroomtype() {
    ...
    // API call
    this.service.add_assettype(data).subscribe(
      (res: any) => {
        this.showSpinner = false;
        if (res.status === 200) {
          Swal.fire({
  ...
  submitCategory(): void {
    ...
    this.subs = true;
    this.service.equipmenttypeadd(data).subscribe(
      (res) => {
        if (res.status === 200) {
          Swal.fire({
            title: 'Done!',
  ...
  saveQuantity() {
    if (this.updateformquantity.invalid) {
      alert("Please enter quantity");
      return;
    }
    const data = {
      quantity: this.updateformquantity.value.update_quantity,
      id: this.dataget.id,
      user_id: localStorage.getItem('user_id'),
      usr_nm: localStorage.getItem('usr_nm'),
    };
    this.service.updateQuantity(data).subscribe(
      (res: any) => {
  ...
  submitProduct(): void {
    ...
    this.subs = true;
    this.service.producttypeadd(data).subscribe(
      (res) => {
        if (res.status === 200) {
          Swal.fire({
            title: 'Done!',
  ...
  editdatesubmit() {
    if (this.updateformrenewal.invalid) {
      ...
      this.service.renweldatasubmit(data).subscribe(res => {
        alert('successfully updated');
        this.getroomtypedata();
        this.showSpinner = false;
        this.addemployeedetailsform.reset();
